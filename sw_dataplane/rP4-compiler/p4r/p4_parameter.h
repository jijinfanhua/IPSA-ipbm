...
std::tuple<std::vector<const P4Parameter*>, std::vector<std::string>> P4Parameter::parse() const {
    if (type == "expression" && expression_value != nullptr) {
        ...
        if (expression_value->op == "not") {
            auto t = expression_value->right->parse();
            auto v = std::get<0>(t);
            auto b = std::get<1>(t);
            return {v, not_apply(b)};
        }
        if (expression_value->op == "and") {
            auto xl = expression_value->left->parse();
            auto xr = expression_value->right->parse();
            return {concat_apply(std::get<0>(xl), std::get<0>(xr)), and_apply(std::get<1>(xl), std::get<1>(xr))};
        }
    ...
    return {std::vector<const P4Parameter*> { this }, std::vector<std::string> { "1" }};
}