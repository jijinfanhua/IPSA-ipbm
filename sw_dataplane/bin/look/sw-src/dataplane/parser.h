...
//            LOG(INFO) << "      field_type: " << (int)trans_fds[i]->fd_type;
    ...
    static uint32_t get_field_int(PHV * phv, FieldInfo * trans_fd/*uint8_t hdr_id, LEN internal_offset, LEN len*/) {
        ...
        if (trans_fd->fd_type == FieldType::VALID) {
            return phv->valid_bitmap[trans_fd->hdr_id];
        }

        uint8_t  hdr_id          = trans_fd->hdr_id;
        uint16_t len             = trans_fd->fd_len;
        auto hdr = phv->parsed_headers[hdr_id];

        int begin_bit = hdr->hdr_offset + trans_fd->internal_offset;

        int begin_byte  = begin_bit / 8; // which byte the beginning of the header at.
        int bit_offset  = begin_bit % 8; // which bit  the beginning of the header at in begin_byte.

        uint32_t res = -(int)((0x80000000) >> (len - 1));
        res >>= bit_offset; // bit string mask.

        res &= *(uint32_t*)(phv->packet + begin_byte);
        res >>= sizeof(uint32_t) - len - bit_offset; // aligned.
...
//                sram_index = 255 - phv->cur_state; // point to default entry
        ...
        phv->bitmap                           += 1 << (31 - sram_entry->hdr_id);
