...
void insert_entry(Parser * parser) {
    ...
    parser->modify_parser_entry_direct(0, 0, 0xffff, 0xffffffff, 0, 112, 1, 1, fis0, -1);

    FieldInfo * fi1 = new FieldInfo{1, 72, 8};
    std::vector<FieldInfo*> fis1;
    fis1.push_back(fi1);
    parser->modify_parser_entry_direct(1, 1, 0x0800, 0xffffffff, 1, 160, 2, 1, fis1, -1);

    std::vector<FieldInfo*> fis2;
    parser->modify_parser_entry_direct(2, 2, 0x6, 0xffffffff, 2, 160, 3, 0, fis2, -1);
