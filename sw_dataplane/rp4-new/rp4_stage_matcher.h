...
public:
    ...
    Rp4SwitchEntry() {}
    Rp4SwitchEntry(std::shared_ptr<Rp4Key> _key, std::shared_ptr<Rp4SwitchValue> _value) : key(std::move(_key)), value(std::move(_value)) {}
    virtual std::string toString() const {
        return "switch-entry";
    }
...
public:
    ...
    Rp4StageMatcher() {}
    Rp4StageMatcher(std::vector<std::shared_ptr<Rp4Operation>> _ops, std::vector<Rp4SwitchEntry> _switch_entries) : ops(std::move(_ops)), switch_entries(std::move(_switch_entries)) {}

    void addEntries(std::vector<Rp4SwitchEntry> &_switch_entries) {
        for (auto entry: _switch_entries) {
            switch_entries.push_back(entry);
        }
        _switch_entries.clear();
    }

    void rebuildOps(std::vector<std::shared_ptr<Rp4Operation>> _ops) {
        ops.clear();
        ops = std::move(_ops);
    }
