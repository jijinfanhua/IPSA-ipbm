...
//update
%token ADD DEL REBUILD IN EXPRESSION OPERATION ACTION_TO_STAGE NEXT_STAGE
...
header_del:
        header_defs struct_defs parser_def dels
    {
        if ($1.size()) {
            ast[num]->header_defs = std::move($1);
            ast[num]->struct_defs = std::move($2);
            ast[num]->parser_def = std::move($3);
...
del:
        DEL STAGE IDENTIFIER
    {
        base[num].stage_manager.logical_stages.erase($3);
    }
    |   DEL TABLE IDENTIFIER IN STAGE IDENTIFIER
    {
        base[num].table_manager.tables.erase($3);
        auto& entries = base[num].stage_manager.lookup($6)->def.matcher.switch_entries;
        auto itr = entries.begin();
        while (itr != entries.end()) {
            if ((*itr).value->isTableStmt() && std::static_pointer_cast<Rp4SwitchTableStmt>((*itr).value)->name == $3) {
                entries.erase(itr);
                break;
            }
            itr++;
    ...
    |   DEL ACTION IDENTIFIER IN STAGE IDENTIFIER
    {
        base[num].action_manager.actions.erase($3);
        auto& entries = base[num].stage_manager.lookup($6)->def.executor.entries;
        auto itr = entries.begin();
        while (itr != entries.end()) {
            if ((*itr).action_name == $3) {
                entries.erase(itr);
                break;
            }
            itr++;
    ...
    |   DEL ENTRIES IN TABLE IDENTIFIER IN STAGE IDENTIFIER
    {
        //ban
    }
    |   DEL PARSER IDENTIFIER IN STAGE IDENTIFIER
    {
        base[num].stage_manager.lookup($6)->def.parser.remove($3);
    }
    |   DEL MATCHER IN STAGE IDENTIFIER key_def
    {
        //ban
    }
    ;
...
rebuild:
        REBUILD DEFAULT_ACTION IDENTIFIER IN TABLE IDENTIFIER IN STAGE IDENTIFIER
    {
        base[num].table_manager.lookup($6)->miss_action_id = base[num].action_manager.lookup($3)->id;
    }
    |   REBUILD EXPRESSION IN ACTION IDENTIFIER IN STAGE IDENTIFIER expressions
    {
        base[num].action_manager.lookup($5)->rebuildExpressions($9);
        base[num].action_manager.reloadPrimitives(&base[num].header_manager);
    }
    |   REBUILD OPERATION IN STAGE IDENTIFIER operations
    {
        base[num].stage_manager.lookup($5)->def.matcher.rebuildOps($6);

    }
    |   REBUILD ACTION_TO_STAGE IDENTIFIER IN STAGE IDENTIFIER NEXT_STAGE IDENTIFIER
    {
        for (auto& entry : base[num].stage_manager.lookup($6)->def.executor.entries) {
            if (entry.action_name == $3)
                entry.stage_name = $8;
        }
    }
    ;
...
add:
        ADD STAGE stage_def
    {
        base[num].stage_manager.addStage(&($3), true);
    }
    |   ADD TABLE table_def
    {
        base[num].table_manager.addTable($3, &base[num].header_manager, &base[num].action_manager);
    }
    |   ADD ACTION action_def
    {
        base[num].action_manager.addAction($3, &base[num].header_manager);
    }
    |   ADD ENTRIES IN TABLE IDENTIFIER IN STAGE IDENTIFIER table_optional_stmt
    {
        //ban
    }
    |   ADD PARSER IDENTIFIER IN STAGE IDENTIFIER
    {
        base[num].stage_manager.lookup($6)->def.parser.add($3);
    }
    |   ADD MATCHER IN STAGE IDENTIFIER switch_entries
    {
        base[num].stage_manager.lookup($5)->def.matcher.addEntries($6);
    }
    |   ADD EXECUTOR IN STAGE IDENTIFIER stage_executor_entries
    {
        base[num].stage_manager.lookup($5)->def.executor.addExecutor($6);
    }
    ;
