...
public:
    ...
    Rp4Binary() {}
    Rp4Binary(Rp4Operator _op, std::shared_ptr<Rp4Operation> _left, std::shared_ptr<Rp4Operation> _right) : op(_op), left(std::move(_left)), right(std::move(_right)) {}
    virtual std::string toString() const {
...
public:
    ...
    Rp4Expression() {}
    Rp4Expression(std::shared_ptr<Rp4LValue> _lvalue, std::shared_ptr<Rp4Operation> _rvalue) : lvalue(std::move(_lvalue)), rvalue(std::move(_rvalue)) {}
    virtual std::string toString() const {
        return "expression";
    }
    virtual std::vector<const Rp4TreeNode *> children() const {
