...
public:
    ...
    IpsaExpression(Rp4Operator _op, std::shared_ptr<IpsaOperand> _left, std::shared_ptr<IpsaOperand> _right) : op(_op), left(std::move(_left)), right(std::move(_right)) {
        type = OPT_EXPR;
    }
