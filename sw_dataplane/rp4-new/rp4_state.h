...
public:
    ...
    int accept_drop = 2;
    Rp4TransitionEntry() {}
    Rp4TransitionEntry(std::shared_ptr<Rp4Key> _key, std::string _next, int _accept_drop = 2) : key(std::move(_key)), next(std::move(_next)), accept_drop(_accept_drop) {}
    virtual std::vector<const Rp4TreeNode *> children() const {
        return {key.get()};
    }
    virtual std::string toString() const {
...
public:
    ...
    Rp4SelectTransition() {}
    Rp4SelectTransition(std::vector<Rp4Field> _targets, std::vector<Rp4TransitionEntry> _entries) : targets(std::move(_targets)), entries(std::move(_entries)) {}
...
public:
    ...
    Rp4StateDef() {}
    Rp4StateDef(std::string _name, Rp4Extract _extract, std::shared_ptr<Rp4Transition> _transition) : name(std::move(_name)), extract(std::move(_extract)), transition(std::move(_transition)) {}
    virtual std::string toString() const {
