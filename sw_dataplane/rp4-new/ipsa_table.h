...
public:
    int action_id;
    int proc_id;
    IpsaActionProcPair(int _action_id, int _proc_id) : action_id(_action_id), proc_id(_proc_id) {}
...
public:
    ...
    explicit IpsaTable(int _table_id, const Rp4TableDef &_table_def) : table_id(_table_id), table_def(_table_def) {}
    //    IpsaTable(const IpsaTable& ) = delete;

    virtual std::shared_ptr<IpsaValue> toIpsaValue() const {
        std::map<std::string, std::shared_ptr<IpsaValue>> dst = {
                {"id", makeValue(id)},
                {"changed", makeValue(changed)},
