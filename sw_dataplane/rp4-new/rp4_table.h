...
public:
    ...
    Rp4MatchEntry() {}
    Rp4MatchEntry(Rp4MatchKey _keys, std::string _action_name) : keys(std::move(_keys)), action_name(std::move(_action_name)) {}
    virtual std::string toString() const {
...
public:
    ...
    Rp4KeyEntry() {}
    Rp4KeyEntry(std::shared_ptr<Rp4LValue> _lvalue, Rp4MatchType _match_type) : lvalue(std::move(_lvalue)), match_type(_match_type) {}
    virtual std::string toString() const {
...
public:
    ...
    Rp4TableDef() {}
    Rp4TableDef(std::string _name, Rp4TableKeyDef _key_def, std::vector<std::shared_ptr<Rp4TableOptionalStmt>> _optional_stmts) : name(std::move(_name)), key_def(std::move(_key_def)), optional_stmts(std::move(_optional_stmts)) {}
    virtual std::string toString() const {
        return "table(" + name + ")";
    }
    virtual std::vector<const Rp4TreeNode *> children() const {
